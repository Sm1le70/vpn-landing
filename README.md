...
- В карточке клиента — **Продлить / сократить**: дни добавляются к текущему сроку, а если подписки нет, она создаётся.
...
docker compose up -d --build
```
Напишите в группе `/chat_id` — бот ответит ID группы (только пока он не указан). Добавьте его и перезапустите ещё раз:
```
TELEGRAM_SUPPORT_CHAT_ID=-100...
```
При запуске сайт сам регистрирует вебхук `https://one.pro-stor.org/webhooks/telegram` (нужен https в `SITE_URL`) и проверяет, что в группе включены темы и у бота есть права. Проблемы видны в `docker compose logs vpn-landing | grep telegram`.
